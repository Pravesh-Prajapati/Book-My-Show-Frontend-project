...
function Seats() {
  ...
  useEffect(() => {
    axios.get("http://localhost:3000/user/" + pos.id)
      .then((res) => {
        ...
        // console.log(getmovieDetail.val);
        ...
        axios.get(`http://localhost:3000/booking/?movieName=${res.data.name}&theaterName=${getmovieDetail.values.name}&date=${getmovieDetail.values.date}&movietime=${getmovieDetail.val}`)
          .then((res) => {
  ...
  let paybtn = async () => {
    let getuserid= JSON.parse(localStorage.getItem("bookmyshowuser"))
    ...
    let bookingdata = {
      ...
      movietime,
      date,
      loginuser:getuserid.email
    }
    // console.log(bookingdata);
    
    // let getseat= await axios.get("http://localhost:3000/booking/?seats="+seats)
    let getseat = await axios.get(`http://localhost:3000/booking/?seats=${seats}&&theaterName=${theaterName}&&movieName=${movieName}&&date=${date}&&movietime=${movietime}`)
